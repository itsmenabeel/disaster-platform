...
/* ─── Status config ─── */
const STATUS_META = {
  pending:    { label: 'Pending Response', color: '#f39c12', bg: 'rgba(243,156,18,0.12)' },
  accepted:   { label: 'Accepted',         color: '#3498db', bg: 'rgba(52,152,219,0.12)' },
  on_the_way: { label: 'On the Way',       color: '#9b59b6', bg: 'rgba(155,89,182,0.12)' },
...
const styles = {
  ...
  backBtn: {
    ...
    padding: '6px 14px', color: 'var(--text-muted)', fontSize: '0.8rem',
  ...
  tag: {
    ...
    borderRadius: '4px', padding: '3px 10px', fontSize: '0.7rem',
    fontFamily: 'IBM Plex Mono, monospace', color: '#3498db',
  ...
  filterBtn: (active) => ({
    padding: '7px 16px', borderRadius: '20px', fontSize: '0.8rem',
    fontFamily: 'Oswald, sans-serif', letterSpacing: '0.04em', fontWeight: 600,
  ...
  card: {
    ...
    borderRadius: 'var(--radius-lg)', marginBottom: '14px',
  ...
  cardNeeds: {
    ...
    fontWeight: 600, letterSpacing: '0.02em', marginBottom: '6px',
  },
  infoRow: {
  ...
  acceptBtn: {
    ...
    fontFamily: 'Oswald, sans-serif', letterSpacing: '0.05em', fontWeight: 600,
  ...
  emptyText: { color: 'var(--text-muted)', fontSize: '0.875rem', marginBottom: '16px' },
};
    ...
    <div style={styles.timeline}>
      {STAGES.map((s, i) => (
        <React.Fragment key={s}>
          <div style={styles.timelineDot(i === idx, i < idx)} title={STATUS_META[s]?.label} />
          {i < STAGES.length - 1 && <div style={styles.timelineLine(i < idx)} />}
        </React.Fragment>
      ))}
    </div>
  );
...
const FILTERS = ['all', 'pending', 'accepted', 'on_the_way', 'completed', 'rejected'];
const FILTER_LABELS = {
  all: 'All', pending: 'Pending', accepted: 'Accepted',
  on_the_way: 'On the Way', completed: 'Completed', rejected: 'Rejected',
};
      ...
      <div style={styles.topBar}>
        <div style={styles.topLeft}>
          ...
          <div>
            ...
            <div style={styles.topSub}>VOLUNTEER PORTAL</div>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <Link to="/volunteer" style={styles.backBtn}>← DASHBOARD</Link>
          <button style={styles.backBtn} onClick={() => { logout(); navigate('/login'); }}>
            SIGN OUT
          </button>
        </div>
      </div>
      ...
      <div style={styles.content}>
        ...
        <div style={styles.pageHeader}>
          ...
          <div style={styles.pageSub}>
            Accept, reject, and update the status of your assigned rescue missions.
          </div>
        </div>
          ...
          {FILTERS.map((f) => {
            ...
            return (
              <button key={f} style={styles.filterBtn(filter === f)} onClick={() => setFilter(f)}>
                ...
                {count > 0 && (
                  <span style={{
                    marginLeft: '6px', background: filter === f ? 'rgba(255,255,255,0.25)' : 'var(--bg-elevated)',
        ...
        {loading ? (
          ...
          <div style={styles.emptyState}>
            ...
            {filter === 'all' && (
              <Link to="/volunteer/map" className="btn-primary" style={{ display: 'inline-flex', width: 'auto', padding: '10px 24px' }}>
                🗺️ VIEW MAP
              </Link>
            )}
          </div>
          ...
          filtered.map((task) => {
            ...
            return (
              <div
                ...
                style={{
                  ...styles.card,
                  borderColor: task.status === 'pending' ? 'rgba(243,156,18,0.4)' : 'var(--border)',
                }}
              >
                {/* Card top */}
                <div style={styles.cardTop}>
                  <div style={styles.cardLeft}>
                    <div style={styles.cardId}>
                      ...
                      <span>·</span>
                      <span>SOS #{sos?._id?.slice(-8).toUpperCase() || '—'}</span>
                      <span>·</span>
                      <span>{new Date(task.createdAt).toLocaleDateString()}</span>
                    </div>

                    <div style={styles.cardNeeds}>
                      {sos?.needs?.map((n) => n.charAt(0).toUpperCase() + n.slice(1)).join(' · ') || 'Unknown needs'}
                    </div>
                    ...
                    <div style={styles.infoRow}>
                      ...
                      {sos?.priority && (
                        <div style={styles.infoItem}>
                          <span>🚨</span>
                          <span style={{
                            color: sos.priority === 'critical' ? '#e63946' : sos.priority === 'high' ? '#f39c12' : 'var(--text-muted)',
                            fontWeight: sos.priority === 'critical' ? 600 : 400,
                      ...
                      {task.acceptedAt && (
                        <div style={styles.infoItem}>
                          <span>✅</span>
                          <span>Accepted {new Date(task.acceptedAt).toLocaleTimeString()}</span>
                        </div>
                      )}
                      {task.completedAt && (
                        <div style={styles.infoItem}>
                  ...
                  <div style={styles.cardFooter}>
                    ...
                    {nextStatuses.map((ns) => (
                      <button
                        ...
                        onMouseEnter={(e) => { e.currentTarget.style.background = 'rgba(52,152,219,0.28)'; }}
                    ...
                    {nextStatuses.length > 0 && (
                      <span style={styles.actionNote}>
                        {task.status === 'accepted' ? 'Begin your journey to victim' : 'Confirm rescue complete'}
                      </span>
                    )}
                  </div>
                )}
