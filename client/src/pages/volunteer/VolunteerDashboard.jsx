...
const styles = {
  ...
  userDot: { width: 7, height: 7, borderRadius: '50%', background: 'var(--success)' },
  roleBadge: {
  ...
  greetingTitle: {
    fontFamily: 'Oswald, sans-serif', fontSize: '1.9rem',
  ...
  navTitle: {
    fontFamily: 'Oswald, sans-serif', fontSize: '1.1rem',
    fontWeight: 600, letterSpacing: '0.03em', marginBottom: '4px',
  },
  ...
  taskId: {
    fontSize: '0.72rem', color: 'var(--text-muted)',
    fontFamily: 'IBM Plex Mono, monospace', marginBottom: '4px',
  },
...
const VolunteerDashboard = () => {
  ...
  const counts = {
    ...
    active:    tasks.filter((t) => ['accepted', 'on_the_way'].includes(t.status)).length,
    ...
    <div style={styles.page}>
      ...
      <div style={styles.topBar}>
        ...
        <div style={styles.topRight}>
          <div style={styles.roleBadge}>VOLUNTEER</div>
      ...
      <div style={styles.content}>
        ...
        <div style={styles.greeting}>
          ...
          <div style={styles.greetingSub}>Here's your rescue operations overview.</div>
        </div>

        {/* Stats */}
        <div style={styles.statsRow}>
          {[
            { label: 'Total Tasks',  value: counts.total,     color: 'var(--text-primary)' },
        ...
        <div style={styles.navGrid}>
          {[
            { to: '/volunteer/tasks', icon: '📋', color: '#e63946', title: 'MY TASKS', desc: 'Accept, reject, and update your rescue assignments' },
            { to: '/volunteer/map',   icon: '🗺️', color: '#3498db', title: 'NEARBY MAP', desc: 'Browse SOS requests near your location' },
          ].map((c) => (
            <Link
              key={c.to} to={c.to}
            ...
            >
              ...
              <div>
                <div style={{ ...styles.navTitle, color: c.color }}>{c.title}</div>
        ...
        <div style={styles.sectionTitle}>
          🕐 RECENT TASKS
          <span style={{ fontSize: '0.72rem', fontWeight: 400, fontFamily: 'IBM Plex Mono, monospace' }}>
            ({tasks.length})
          </span>
        </div>
        ...
        {loading ? (
          ...
          <div style={styles.emptyState}>No tasks assigned yet. Check the map for nearby SOS requests.</div>
        ) : (
          recent.map((task) => {
            ...
            return (
              <div key={task._id} style={styles.recentCard}>
                <div>
                  <div style={styles.taskId}>#{task._id.slice(-8).toUpperCase()}</div>
                  <div style={styles.taskNeeds}>{sos?.needs?.join(', ') || '—'}</div>
                  <div style={styles.taskDate}>{new Date(task.createdAt).toLocaleString()}</div>
                </div>
                <div style={{
        ...
        {recent.length > 0 && (
          <Link to="/volunteer/tasks" style={{ display: 'block', textAlign: 'center', marginTop: '12px', fontSize: '0.82rem', color: 'var(--text-muted)', fontFamily: 'IBM Plex Mono, monospace' }}>
            VIEW ALL TASKS →
          </Link>
        )}
      </div>
